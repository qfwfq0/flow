...
! per calcolo lineare da equilibrio ohmico
       REAL (8) :: BREQ(0:LX),  BTEQ(-1:LX),  BZEQ(-1:LX)   
     .       , VREQ(0:LX),  VTEQ(-1:LX),  VZEQ(-1:LX)   

!#! Marco, ZEIT
       real(8) :: zeit
!#! Marco, for input-output
       character(len=255) :: prefix, simdir
       character(len=255) :: un_all, un_end, un_prevend
       character(len=255) :: unit_all, unit_end, unit_prevend
       CHARACTER(len=255) :: cwd

! daniele, 09/05/2007
! per calcolo dell'equilibrio ohmico screw pinch come in PIXIE3D
       REAL (8) :: ohmbt(0:LX+1), ohmbz(0:LX+1)
       REAL (8) :: ohmbt_old(0:LX+1), ohmbz_old(0:LX+1)
       REAL (8) :: dummy(0:LX+1)
       REAL (8) :: VR0(0:LX)
       REAL (8) :: vz0(0:LX)
       real (8) :: lxx, one
...
!#! Marco, 03/12/2014 for irrotational MP
       real(8) :: dumr,dumt,dumz
       complex(8) , dimension(0:lx) :: bbr
       complex(8) , dimension(-1:lx):: bbt, bbz
       integer , parameter :: uft = 4187
       integer :: statuss
       integer :: ik, loc, tempmv
       real(8) :: temp_ph_mp, amp_old, phold
       logical :: file_exist
      ...
      include 'files.inc.for'
   
       write(6,*) ' '
       write(6,*) ' eta0 = ', eta0,'  mue = ',mue
       write(6,*) ' aspect ratio',rr
       ...
       if (i2d.eq.1) write(6,*) ' i2d,heli2d,m2d,n2d,my: ',
     .        i2d,heli2d,m2d,n2d,my
       write(6,*) ' IBAND= ',IBAND
       write(6,*) ' inopert= ',inopert
       write(6,*) ' inoconv, ippcd, ilinear = ',inoconv,ippcd,ilinear
       ...
       write(6,*) ' fasi sganciate? ',fasi_sganciate
       write(6,*) ' irrotational MP? ',irrot
       write(6,*) ' MP = ',mp_perc,' MP phase = ',ph_mp
       write(6,*) ' mMP = ',m_mp,' nMP = ',n_mp
...
c               - ecluso controllo sul flusso
       ...
       JANZ(0,0) = 0                                                    
       J0 = JANZ(0,0)                                                   
!#! Marco, 08/02/2017 ho eliminato l'inutile N0,
! lo sostituisco con NZ(0)
!       IF(N0.EQ.0) GO TO 11  
       IF(NZ(0).EQ.0) GO TO 11  
...
!#! Marco, gennaio 2017, riscrivo il codice per creare la MESH
!#! quindi commento fino al tag @finecommento
...
!#! @finecommento
!#! Marco, codice nuovo per creare le mesh X, X2
       ZS = lx       
       do ix = 0, lx 
        zs2(ix) = lx / 2
        zs22(ix) = lx 
...
!...................... INITIAL EQUILIBRIUM .....................   
       if (ipr .eq. -2) then                               
        call compute_equil(x,x2,bt0,bz0,vr0)
...
!     .        , ' POWALL = ',P0WALL
 ...
 1000   FORMAT(I3,' ',f6.3,' ',e12.5,' ',e12.5,' ',e12.5)
        ...
        write(*,*) 'other equilibria not yet implemented!'
        stop
       endif
...
!#!78 CONTINUE 
       ...
       unit_all = trim(trim(prefix)//trim(simdir)//"dat/"//trim(un_all))
       unit_end = trim(trim(prefix)//trim(simdir)//"dat/"//trim(un_end))
       unit_prevend = trim(trim(prefix)//trim(simdir)
     .                //"dat/"//trim(un_prevend))
       write(*,*) 'debug_files_0 ',trim(unit_all)
       write(*,*) 'debug_files_0 ',trim(unit_end)
       write(*,*) 'debug_files_0 ',trim(unit_prevend)
...
250   CONTINUE          
         ...
         write(*,*) 'debug lettura_end',bt(20,0)
        close(unit=ibein) 
...
991    CONTINUE
     ...
     .        '   REAL(BT(LX,J0)) = ', REAL(BT(LX,J0))
       WRITE(6,*) '   REAL(BR(LX-1,J0)) =',REAL(BR(LX-1,J0)),
     .        '   REAL(BR(LX,J0)) = ', REAL(BR(LX,J0))
!        WRITE(6,*) 'real(br(lx-1,73)) =',real(br(lx,73)),
!     .  'aimag(br(lx,73)) = ', aimag(br(lx,73))
       call flush(6)
...
! tengo solo la pert in vr, come in pixie e la metto come in pixie
          VR(0,J) = (0.,0.)
          write(*,*) 'VR(IX,J)=cmplx(0.5*EPS1/X(IX)*?*X(IX))**3.,0.)'
          DO 36 IX=1,LX
          call flush(6)
...
!          IN1=N2D
         ...
         ENDIF
             DO 1113 N=IN0,IN1
                WRITE(6,*) 'perturbo 1113 il modo',M,N,EPS1
...
!#! inizioparte
      ...
      do idx=0, size(n_mp,1)-1
       if (n_mp(idx) .eq. 0 .and. mp_perc(idx) .ne. 0.d0) then
        exit
       endif
       write(*,*) m_mp,n_mp(idx),janz(m_mp,n_mp(idx)),RR
       j_mp(idx) = janz(m_mp,n_mp(idx))
       j_mpold(idx) = janz(m_mpold,n_mpold(idx))
       amp_a(idx) = 2.d0 * mp_perc(idx) / (BESSI(m_mp+1, n_mp(idx)/RR)
     .                           +  BESSI(m_mp-1, n_mp(idx)/RR))
       if (mp_perc(0) .ne. 0.d0) then      
...
!       write(*,*) 'DEBUG-1',j_mp,j_mpold
     ...
     .               ,m_mpold,',',n_mpold(idx),') ',j_mpold(idx)
          call flush(6)
          write(*,*) 'INIZIO LA SIMULAZIONE E APPLICO MP IRROT'
          call apply_irrot_bc(mp_perc(idx),amp_a(idx),m_mp,n_mp(idx)
     .                       ,ph_mp(idx),bbr,bbt,bbz)
          do ix=0, lx 
...
!       !diventa meno pi/2
     ...
     .               / (BESSI(m_mpold+1, n_mpold(idx)/RR)
     .               +  BESSI(m_mpold-1, n_mpold(idx)/RR))
           call apply_irrot_bc(ampold(idx),amp_old,m_mpold,
...
!#! Marco, 07/02/2017, rimuovo l'implicit open dei files presente in
!SpeCyl
...
!#! Marco, apri il file specyl_?_end.dat        
     ...
     .        '   REAL(BZ(LX,J0)) = ', REAL(BZ(LX,J0))
       WRITE(6,*) '   REAL(BT(LX-1,J0)) =',REAL(BT(LX-1,J0)),
     .        '   REAL(BT(LX,J0)) = ', REAL(BT(LX,J0))
       WRITE(6,*) 'real(br(lx-1,j_mp(0))) =',real(br(lx,j_mp(0))),
     . 'aimag(br(lx,j_mp(0))) = ', aimag(br(lx,j_mp(0))),
     . j_mp(0)
...
68       J1 = JANZ(-M,-N)                                                 
!       								
!!      write(*,*) 'end of computation after first faraday',j,j1
!!      write(*,*) btn(50,j0), bzn(50,j0),brn(50,j0)
!!     .             ,'bt=',bt(50,j0),' bz=',bz(50,j0),br(50,j0)
!!  BR AUS BT +BZ ........................                               
       BRN(0,J) =(0.0,0.0)                                              
       DO 149 IX=1,LX-1                                                 
...
12     CONTINUE                                                          
      ...
      if (it .eq. 10) then
      WRITE(6,*) it,'a   br(lx,73) =',br(lx,73)
     .           ,'   jr(lx,73) =',jr(lx,73)
      endif
!!      write(*,*) 'end of computation after first faraday + solen' 
!!      write(*,*) btn(50,j0), bzn(50,j0),brn(50,j0)
!!     .             ,'bt=',bt(50,j0),' bz=',bz(50,j0),br(50,j0)
!!      WRITE(6,*) '   br(lx,73) =',br(lx,73),
!!     .           '   brn(lx,73) =',brn(lx,73)
!       !#! Marco
...
352     CONTINUE                                                          
15       CONTINUE  !#! fine ciclo in m,n
!      write(*,*) 'end of computation after seminplicito' 
!      write(*,*) btn(50,j0), bzn(50,j0),brn(50,j0)
!     .         ,bt(50,j0),bz(50,j0),br(50,j0)
!     .         ,vt(50,j0),vz(50,j0),vr(50,j0)
!     .         ,vtn(50,j0),vzn(50,j0),vrn(50,j0)
!      WRITE(6,*) '   br(lx,73) =',br(lx,73),
!     .           '   brn(lx,73) =',brn(lx,73)
...
!     .            ,brn(ix,j), vt(ix,j),br(ix,j),vtn(ix,j)
!       endif
!!#! Marco
...
!          write(*,*)  'dentro seconda faraday,it= ',it,m,n
!!!     .          'brn=',brn(50,j0),' br=',br(50,j0),
!     .          ,'btn=',btn(50,j0),' bt=',bt(50,j0)
!     .          ,'bzn=',bzn(50,j0),' bz=',bz(50,j0)
!     .          ,'brn=',brn(50,j0),' br=',br(50,j0)
...
C     .............                                                     
!       if (j .eq. j0) then
!      write(*,*) 'end of computation after second faraday' 
!      write(*,*) btn(50,j), bzn(50,j)
!     .      ,bt(50,j),bz(50,j)
!       endif
...
73     IF(J.EQ.0) GO TO 82                                               
       ...
       enddo 
82     CONTINUE  
28     CONTINUE  !#! fine ciclo in M,N
!!      write(*,*) 'end of computation after second faraday + solen' 
!!      write(*,*) btn(50,j0), bzn(50,j0)
!!     .      ,bt(50,j),bz(50,j)
!!      WRITE(6,*) '   br(lx,73) =',br(lx,73),
!!     .           '   brn(lx,73) =',brn(lx,73)
!!#! Marco
...
!viene fatta, in quel caso, più avanti a riga circa 3320
...
!           write(*,*) 'apply_bc_final',loc,phmp(loc)
     ...
     .              * (BESSI(m_mp+1, n_mp(loc)/RR*X(LX))
     .              +  BESSI(m_mp-1, n_mp(loc)/RR*X(LX)) ) )**2.d0)
...
!           write(*,*) 'end of computation after first imposition of bc' 
!           WRITE(6,*) '   UU(1,lx) =',uu(1,lx)
...
!           if (j .eq. 73) then
!            write(*,*) 'end of comput after bc / Rot',phmp(loc)
!            WRITE(6,*) '   UU(1,lx) =',uu(1,lx)
!           endif
...
C      .................                                                
!         write(*,*) 'in mezzo al calcolo semimplic'
!         write(*,*) btn(50,j0), bzn(50,j0)
!         write(*,*) bt(50,j0), bz(50,j0)
         ELSE !(J ne 0)
...
!        write(*,*),'prima solenoidalita2 ,it= ',it,j
!     .             ,'btn=',btn(50,j0),' bt=',bt(50,j0)
!     .             ,'bzn=',bzn(50,j0),' bz=',bz(50,j0)
C  BR AUS BT +BZ                              
!       if (j .eq. 73) then                          
!        write(*,*) 'passo per questa solen0'
!       endif 

       BRN(0,J) =(0.0,0.0)                                              
       DO 151 IX=1,LX-1                                                 
!#! Marco, calcolo anche il punto LX della mesh
!       DO 151 IX=1,LX                         
       BRN(IX,J)=(I/ZS22(IX-1)*(-BTN(IX-1,J)*M-BZN(IX-1,J)*N*X2(IX-1)   
     . /RR)+BRN(IX-1,J)*X(IX-1))/X(IX)                                  
151     CONTINUE                                                          
!      if (j .eq. 73) then                          
!        write(*,*) 'passo per questa solen1',brn(90:lx,73)
!       endif
...
404   CONTINUE                                                          
83    CONTINUE                                                          
41     CONTINUE  !fine del ciclo in M,N
...
!(che nelle simulazioni è dell'ordine di  10^-19
        ...
        enddo
!      write(*,*) 'end of computation after boundary cond', it 
!      WRITE(6,*) '   br(lx,73) =',br(lx,73),
!     .           '   brn(lx,73) =',brn(lx,73),abs(brn(lx,73))
...
!             write(*,*) 'ph-1',it,phmp(idx)
!             write(*,*) 'get-1',get_phase(brn(lx,j_mp(idx)))
!            endif
            call apply_irrot_bc(mp_perc(idx),amp_a(idx),m_mp
...
!             write(*,*) 'add1',bbr(lx-1),bbr(lx)
!             write(*,*) 'add11',bbz(lx+1),bbz(lx)
!            endif
!#! 
!            if (get_phase(bzn(lx+1,j_mp(idx)))
!     .           - get_phase(brn(lx,j_mp(idx))) .gt. 2.d0) then
!             write(*,*) 'DIAG_deltaph'
!     .      ,get_phase(bzn(lx+1,j_mp(idx)))-get_phase(brn(lx,j_mp(idx)))
!            endif
...
!  103 FORMAT(E12.4,2I4)                                                 
       ...
       END program specyl
!#!Marco, 20-11-2015: commento questa parte e inserisco il contenuto di
!#!eq_csha.blc.for in cyl1.for
