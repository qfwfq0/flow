...
! per calcolo dell'equilibrio ohmico screw pinch come in PIXIE3D
       ...
       REAL (wp) :: evr0(0:LX),ebt0(-1:lx),ebz0(-1:lx)
       REAL (wp) :: vr0(0:lx),vz0(0:LX)
...
!#! inizioparteMP_Marco
       ...
       write(*,*) 'non ci sono',m_mp(idx),n_mp(idx),mp_perc(idx)
...
!           write(*,*) 'apply_bc_final',loc,phmp(loc)
     ...
     .              * (BESSI(m_mp(loc)+1, n_mp(loc)/RR*X(LX))
     .              +  BESSI(m_mp(loc)-1, n_mp(loc)/RR*X(LX)) ) )**2.d0)
