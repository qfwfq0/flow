...
!#! Marco, define meshes
         ...
         sign_r = signr(ph_mpr)
         sign_t = signt(ph_mpr)
!         write(*,*) 'DIAG_apply_before',ph_mpr,sign_r,sign_t
         do ix=0, lx
     ...
     .               ,m_mpold,',',n_mpold(idx),') ',j_mpold(idx)
        call flush(6)
         if (mp_perc(idx) .ne. 0.d0) then !(@3)
          ...
          call apply_irrot_bc(lx,mp_perc(idx),amp_a(idx),m_mp,n_mp(idx)
...
!          write(*,*) 'check amplitude', abs(bbr(lx)),abs(br(lx,73))
     ...
     .                 /real(br(lx,j_mp(idx))) ) 
!       !#! Marco (spiegazione nuova) metto +pi/2.d0 perché la fase è
...
!       !diventa meno pi/2
           ...
           write(*,*) 'DIAG_rot_MP, aimag,real', real(br(lx,j_mp(idx)))
     .                     , aimag(br(lx,j_mp(idx)))
           write(*,*) 'Rotating MP, the old phase is',
     .                 phmp(idx)
           write(*,*) 'Rotating MP, the old br phase is', 
     .                 phmp(idx) - pi/2.d0
     ...
     .               +  BESSI(m_mpold-1, n_mpold(idx)/RR))
           phold = phmp(idx)
           call apply_irrot_bc(lx,ampold(idx),amp_old,m_mpold,
...
!viene fatta, in quel caso, più avanti a riga circa 3320
         if (irrot .eq. .true. .and. mp_perc(0) .ne. 0.d0) then !#!@1
...
!           WRITE(6,*) '   UU(1,lx) =',uu(1,lx)
          else !#! (br_pert_freq /= 0.d0)
!#! attenzione, solo se it = 1
           call flush(6)
!#! inizioparte3
!           if (it .eq. 1 .and. any(j_mp .eq. j)) then
!            do ik=0, size(n_mp)-1
!             temp_mp(ik) = abs(j_mp(ik)-j)
!            end do
!            loc = minloc(temp_mp,1) - 1
!!#! Marco, visto che sto imponendo le bc su br aggiungo pi/2 alla fase,
!!per costruzione
!            phmp(loc) = phmp(loc) + pi/2.d0 
!            if (phmp(loc) .gt. 2.*pi) then
!             phmp(loc) = phmp(loc) - 2.*pi
!            endif
!            sign_r = signr(phmp(loc))
!            write(*,*) 'apply_bc_br_pert_freq',loc,phmp(loc),'it',it
!            dumr = sqrt(( amp_a(loc) * 0.5d0 
!     .              * (BESSI(m_mp+1, n_mp(loc)/RR*X(LX))
!     .              +  BESSI(m_mp-1, n_mp(loc)/RR*X(LX)) ) )**2.d0)
!
...
!     .           / (1.d0 + tan(phmp(loc))**2.d0) ))
!            endif
!           write(*,*) 'end of comput after first imposition of rot bc' 
!           WRITE(6,*) '   UU(1,lx) =',uu(1,lx)
!!#! fineparte3
!           endif
          endif!#! (end of br_pert_freq /= 0.d0)
         endif
...
151     CONTINUE                                                          
!       if (j .eq. 73) then                          
!        write(*,*) 'passo per questa solen',brn(lx,73)
!       endif
...
c        ENDIF
       ...
       ENDIF
         ...
         if (irrot .eq. .true. .and. mp_perc(0) .ne. 0.d0
...
!#! inizioparte2
           do idx=0, size(n_mp) -1
            ...
            call apply_irrot_bc(lx,mp_perc(idx),amp_a(idx),m_mp
