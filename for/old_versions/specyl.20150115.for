...
!#! Marco, subroutines and functions
      contains
        subroutine apply_irrot_bc(lx,amp_a,m_mp,n_mp,ph_mp,bbr,bbt,bbz)
         ...
         real(wp)  :: rr, pi
         real(wp) , dimension(0:lx) :: x
         real(wp) , dimension(-1:lx) :: x2
         integer :: ix
...
!#! Marco, define meshes
     ...
     .           * (BESSI(m_mp+1, n_mp/RR*X(IX)) 
     .           +  BESSI(m_mp-1,n_mp/RR*X(IX))))**2.d0
     .           / (1.d0+tan(ph_mp-pi/2.d0)**2.d0))
          bbr(ix) = cmplx(sign_r * dumr
     .                       , tan(ph_mp-pi/2.d0) * sign_r * dumr)
         enddo
         do ix=0, lx
          dumt = sqrt((1.d0/X2(IX) * amp_a * m_mp
     .                 * RR / n_mp * BESSI(m_mp, n_mp/RR*X2(IX)))**2. 
     .                 / (1.d0 + tan(ph_mp)**2.d0) )
          dumz = sqrt((amp_a 
     .                 * BESSI(m_mp, n_mp/RR*X2(IX)))**2.
     .                 / (1.d0 + tan(ph_mp)**2.d0) )
!          write(*,*) ix,dumz
          bbt(ix) = cmplx(sign_t * dumt
     .                       , tan(ph_mp) * sign_t * dumt)
!#!Marco, 13/1/15: il segno meno perché *assumo* che n_mp<0, metto la
!cosa corretta
          bbz(ix) = cmplx(sign(1,n_mp) * sign_t * dumz
     .            , sign(1,n_mp) *  tan(ph_mp) * sign_t * dumz)
         enddo
...
!field components, depending on the value of ph_mp
       implicit none
       integer , parameter :: wp = kind(1.d0) ! working precision
       real (wp) , intent(in) :: phmp
       real (wp) :: signr, pi
       DATA   pi/3.14159265358979323846d0/
        if (phmp .ge. 0. .and. phmp .lt. pi/2.d0) then
         signr = + 1. 
...
!
      ...
      integer , parameter :: wp = kind(1.d0) ! working precision
      REAL(wp) :: X,BESSI,TOX,BIM,BI,BIP
      IF (N.EQ.0) THEN
      BESSI = BESSI0(X)
      RETURN
      ENDIF
...
! Auxiliary Bessel functions for N=0, N=1
      FUNCTION BESSI0(X)
      integer , parameter :: wp = kind(1.d0) ! working precision
      REAL(wp) :: X,BESSI0,Y,P1,P2,P3,P4,P5,P6,P7,
...
! ----------------------------------------------------------------------
      FUNCTION BESSI1(X)
      integer , parameter :: wp = kind(1.d0) ! working precision
      REAL(wp) :: X,BESSI1,Y,P1,P2,P3,P4,P5,P6,P7,
     .     Q1,Q2,Q3,Q4,Q5,Q6,Q7,Q8,Q9,AX,BX
...
! Daniele, 16/05/2007
          ...
          BT(IX,J0) = BT0(IX)
          VR(IX,J0) = VR0(IX)
!          WRITE (6,1002) X2(IX), BZ(IX,J0), BT(IX,J0)                     
...
! e la metto come in pixie
     ...
     .                         1./3.*0.5*EPS1/X(IX)*sin(pi*X(IX))**3.)
                   write(*,*) 'VR(IX,J) =  cmplx(0.5*EPS1/X(IX)*sin(pi*X(IX))**3.,
     .                         1./3.*0.5*EPS1/X(IX)*sin(pi*X(IX))**3.)'
...
!         write(*,*) 'size',size(bbr),size(bbt),size(bbz)
         do ix=0, lx 
          br(ix,j_mp) = bbr(ix)
...
!         write(*,*) 'it=',it
        if (it .lt. 3) then 
         write(*,*) wp, sizeof(sum3),sizeof(zs2)
         write(*,*) precision(sum3), range(sum3)
         write(*,*) maxexponent(zs2), minexponent(zs2)
...
!       endif
   26 CONTINUE                                                          
        if (it .lt. 33) then
         if (jmn .eq. 0) then
        write(*,*)  '1_far: ',js,j,j1
     .           ,vt(kkix,j)*bz(kkix,j1), vz(kkix,j)*bt(kkix,j1)
     .           ,vt(kkix,j)*bz(kkix,j1)- vz(kkix,j)*bt(kkix,j1)
     .           , sum3(kkix)
!     .           ,sum3(kkix),sum0(kkix),sum1(kkix)
!     .           ,br(kkix,j), vr(kkix,j)
!     .           ,br(kkix,j1), vr(kkix,j1)
!     .           ,bt(kkix,j), vt(kkix,j)
!     .           ,bt(kkix,j1), vt(kkix,j1)
!     .           ,bz(kkix,j), vz(kkix,j)
!     .           ,bz(kkix,j1), vz(kkix,j1)
          endif
        endif
!   57 CONTINUE                                                          
   56 CONTINUE
...
68     J1 = JANZ(-M,-N)                                                 
        ...
        write(*,*),'0_sol,it= ',it
     .             ,'btn=',btn(kkix,j0),' bt=',bt(kkix,j0)
     .             ,'bzn=',bzn(kkix,j0),' bz=',bz(kkix,j0)
!  BR AUS BT +BZ ........................                               
       BRN(0,J) =(0.0,0.0)                                              
       DO 149 IX=1,LX-1                                                 
...
!          write(*,*) 'ph2',phmp
     ...
     .           / (1.d0+tan(phmp-pi/2.d0)**2.d0))
           UU(1,lx) = cmplx(sign_r * dumr
     .                       , tan(phmp-pi/2.d0) * sign_r * dumr)
          endif
         endif
...
!#! Marco, add irrotational perturbation 
        ...
        if (it .lt. 3) then 
         write(*,*) 'alla fine', it
     .            ,bt(kkix,0),btn(kkix,0)
     .            ,bz(kkix,0),bzn(kkix,0)
        endif
...
!       write(*,*) 'ph_mp = ',phmp
!       if (br_pert_freq /= 0.d0) then
!       ph_mp_file='/ricercatori/ft/specyl/veranda/flow/ph_mp.bin'
!       file_exist = .false.
!       inquire(file=trim(ph_mp_file), exist=file_exist)
!       write(*,*) 'file_exist=',file_exist, trim(ph_mp_file),it
!         open(unit=uft,file=trim(ph_mp_file),status='unknown',
...
!#! Marco, write last value of MP phases, if irrot .eq. true
!         if (br_pert_freq /= 0.d0) then
!          ph_mp_file='../dat/ph_mp.bin'
!          file_exist = .false.
!          inquire(file=trim(ph_mp_file), exist=file_exist)
!          write(*,*) 'file_exist=',file_exist, trim(ph_mp_file)
!          if (file_exist .eq. .true.) then
!           open(unit=uft,file=trim(ph_mp_file),status='replace',
!     .         form='unformatted',action='write')
!           write(uft) phmp
!           close(uft) 
!          else
!          write(*,*) 'open',trim(ph_mp_file),it
!          call flush(6)
!           open(unit=uft,file=trim(ph_mp_file),status='unknown',
!     .         form='unformatted', action='write')
!           write(uft) phmp
