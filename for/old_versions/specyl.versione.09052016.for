...
!#! Marco, subroutines and functions
       contains
        subroutine apply_irrot_bc(lx,amp_ar,m_mpr,n_mpr
     .                           ,ph_mpr,bbr,bbt,bbz)
...
!PIXIE3D
!#! Marco, 09-05-2016: provo a togliere il segno meno
         bbt = - bbt
         bbz = - bbz
...
!       write(*,*) 'DEBUG-1',j_mp,j_mpold
     ...
     .               ,m_mp,',',n_mp(idx),') ',j_mp(idx)
        ...
        write(*,*) mp_perc(idx), ph_mp(idx), 'normalized amplitude', amp_a(idx)
       ...
       !irrotational MP of the selected helicity
        if (.not. IBAND) then !(@2)
         if (mp_perc(idx) .ne. 0.d0) then !(@3)
          ...
          call apply_irrot_bc(lx,amp_a(idx),m_mp,n_mp(idx)
...
!       !diventa meno pi/2
           ...
           write(*,*) 'DIAG_rot_MP, aimag,real', aimag(br(lx,j_mp(idx)))
     .                     , real(br(lx,j_mp(idx)))
     ...
     .                        ,phmp(idx),bbr,bbt,bbz)
           ...
           do ix=-1, lx 
            ...
            bz(ix,-j_mp(idx)) = conjg(bz(ix,j_mp(idx)))
           enddo
           write(*,*) 'after sub - add', aimag(br(lx,j_mp(idx)))
     .                     , real(br(lx,j_mp(idx)))
...
!#! Marco, attenzione per MP irrotazionali
         if (irrot .eq. .true. .and. mp_perc(0) .ne. 0.d0) then
