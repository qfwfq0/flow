...
!       DT2 = DT * (1-ALF)
      ...
      WRITE(6,'(a,i3,a,i3,a,i3,a,i2)') 'M',M,' N',N,
...
!                   vr(ix,j) = vr(ix,j) + (1e-7,2e-7)*dble(n)
                 write(*,*) vr(ix,j)
!#! Marco 06/08/2014
...
!#! cambio la fase anche di VT
 36             CONTINUE
 113         CONTINUE
!             DO 1113 N=IN0-1,IN1-1,3
!                WRITE(6,*) 'perturbo il modo',M,N
!                J = JANZ(M,N)
!                write(*,*) 'j del modo perturbato',j
!                write(*,*) 'VR(IX,J)=0.5*EPS1/X(IX)*?*X(IX))**3.'
!                call flush(6)
!                J1 = JANZ(-M,-N)
!                IF(J .EQ. J0) GO TO 113
!! daniele,10/12/2008
...
!                J = JANZ(M,N)
!                write(*,*) 'j del modo perturbato',j
!                write(*,*) 'VR(IX,J)=1.2*I*0.5*EPS1/X(IX)*?*X(IX))**3.'
!                call flush(6)
!                J1 = JANZ(-M,-N)
!                IF(J .EQ. J0) GO TO 113
!! daniele,10/12/2008
...
!                VR(0,J) = (0.,0.)
!                DO 1236 IX=1,LX
!                   VR(IX,J) =  1.2*I*0.5*EPS1/X(IX)*sin(pi*X(IX))**3.
!!                   vr(ix,j) = vr(ix,j) + (1e-7,2e-7)*dble(n)
!                 write(*,*) vr(ix,j)
!!#! Marco 06/08/2014
!!#! cambio la perturbazione iniziale, aggiungendo una fase di pi/2
...
!#! Marco
!       if (m .eq. 1 .and. n .eq. -7 .and. ix .eq. 50) then
...
!        IF (J.EQ.113) UU(1,LX) = (0.0,-6.0E-3)
        !! modo m=1,n=-7
!         IF (J.EQ.74) UU(1,LX) = (0.0,-2.25E-3)
