...
! ma questo vale solo nel caso lambda=1!
             ...
             bz00(IX) = B0                                                
!             bt00(IX) = X2(IX)*bz00(IX)/((1.0+(X2(IX)/C1)**2)*RR*Q0)
! Daniele, da quando ho iniziato a usare lambda (settembre 2006)
             bt00(IX) = X2(IX)*bz00(IX)/(RR*Q0*
...
! NB rimane RS = 0.4 ma non mi cambia nulla! (conta solo se è zero o non 0)
       IF (IPR.GE.0.AND.IZ.GT.0) THEN
         ...
         DRAD = 1./(NRAD - 1)                                           
         RAD = 0.0                                                      
         CALL EQUIL(RAD,BZ000,BT000,P010)                                 
         F0 = M*THETA0 + N/RR*BZ000
         DO 785 IRAD=2,NRAD                                             
              RAD = (IRAD - 1)*DRAD                                     
              CALL EQUIL(RAD,BZ000,BT000,P010)                            
              F01 = M/RAD*BT000 + N/RR*BZ000
              DUM = F0*F01
              IF (DUM.LE.0.0) THEN                                      
...
!     .             cos(im*n2d*0.2d0),EPS1/X(IX)*sin(pi*X(IX))**3.*
!     .             sin(im*n2d*0.2d0))
...
!#! Marco, 19/01/15: if simulation is in restart mode ADD the
!irrotational MP to the previous field
          else
           write(*,*) 'CONTINUO LA SIMULAZIONE E APPLICO MP IRROT'
  
!            call flush(6)
!!#! Marco, subtract old irrotational perturbation 
!!#! dealing with a purely imaginary radial component of the field
!            if (real(br(lx,j_mp)) .eq. 0.d0) then
!             write(*,*) 'ph=NaN'
!             if (aimag(br(lx,j_mp)) .lt. 0. ) then
!              phmp = 0
!             else
!              phmp = pi
!             endif
!            else
!!#! dealing with a generic radial component of the field
!            phmp = atan( aimag(br(lx,j_mp))/real(br(lx,j_mp)) )
!            endif
!            ampold = (aimag(br(lx,j_mp))**2.d0
!     .                  + real(br(lx,j_mp))**2.d0)
            amp_old = 2.d0 * ampold / (BESSI(m_mp+1, n_mp/RR)
     .                           +  BESSI(m_mp-1, n_mp/RR))
            write(*,*) 'old phase',phold,amp_old
            if (ampold .eq. 0.d0) then
             write(*,*) 'nothing to subtract'
            else
             call apply_irrot_bc(lx,amp_old,m_mp,n_mp,phold,bbr,bbt,bbz)
             do ix=0, lx 
...
!          write(*,*) 'ph2',j,phmp
     ...
     .                        , -1.d0 * sign_r * dumr)
            ...
            dumr = sqrt((amp_a * 0.5d0 * tan(phmp)
     .            * (BESSI(m_mp+1, n_mp/RR*X(LX)) 
     .            +  BESSI(m_mp-1,n_mp/RR*X(LX))))**2.d0
     .            / (1.d0+tan(phmp)**2.d0))
            if (abs(phmp-pi) .lt. pitol) then
             UU(1,lx) = cmplx(0.d0
     .                       ,  + 1.d0 * sign_r * dumr)
            else
...
1111         write(6,*) ' zeit= ',zeit,'  stato finale:'
  ...
  550 FORMAT(1X,' ITEND,ITP=',2I10,' ZEIT=',1PE12.4,' RS=',E12.4)       
       END                                                              
!#! Marco, end of SpeCyl
!#! Marco, if included inside the main code it does not work
