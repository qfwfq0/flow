!#! Marco, versione 20160127
      module mp

!#! Marco, subroutines and functions
       contains
        subroutine apply_irrot_bc(amp_ar,m_mpr,n_mpr,ph_mpr,bbr,bbt,bbz)

!         implicit none
         include 'cyl1.inc.for'
!         integer , parameter :: wp = kind(1.d0) ! working precision
!         integer , intent(in) :: lx
!         integer, intent(in)   :: m_mpr, n_mpr
...
!         write(*,*) 'prova RR module',rr,wp,lx,m_mpr,n_mpr,i,pi,pitol
!#! Marco, define meshes
         dx = real(1.d0/lx,wp)
         do ix=0, lx
...
!         write(*,*) 'before',ph_mpr
     ...
     .               , (- 1.d0) / tan(ph_mpr) * sign_r * dumr)
     .               * ( sqrt( tan(ph_mpr)**2.d0   
     .               / (1.d0 + tan(ph_mpr)**2.d0) ))
          endif
         enddo

         do ix=0, lx
          dumt = sqrt((1.d0/axx2(IX) * amp_ar * m_mpr
     .           * RR / n_mpr * BESSI(m_mpr, n_mpr/RR*axx2(IX)))**2. )
          dumz = sqrt((amp_ar 
     .           * BESSI(m_mpr, n_mpr/RR*axx2(IX)))**2. )
     ...
     .            , sign(1,n_mpr) * tan(ph_mpr) * sign_t * dumt)
     .            / sqrt(1.d0 + tan(ph_mpr)**2.d0) 
           bbz(ix) = cmplx(  sign_t * dumz
     .            ,  tan(ph_mpr) * sign_t * dumz)
     .            / sqrt((1.d0 + tan(ph_mpr)**2.d0)) 
          endif
         enddo
...
!#! Marco, allocate big arrays
       allocate(BR(0:LX,-IZ:IZ),BT(-1:LX,-IZ:IZ)
     .             ,BZ(-1:LX,-IZ:IZ))
       allocate(VR(0:LX,-IZ:IZ),VT(-1:LX,-IZ:IZ)
     .             ,VZ(-1:LX,-IZ:IZ))  
       allocate(BRN(0:LX,-IZ:IZ),BTN(-1:LX,-IZ:IZ)
     .             ,BZN(-1:LX,-IZ:IZ))   
       allocate(VRN(0:LX,-IZ:IZ),VTN(-1:LX,-IZ:IZ)
     .             ,VZN(-1:LX,-IZ:IZ))
       allocate(VRP(0:LX,-IZ:IZ),VTP(-1:LX,-IZ:IZ))
       allocate(JR(-1:LX,-IZ:IZ),JT(0:LX,-IZ:IZ)
     .             ,JZ(0:LX,-IZ:IZ))
       allocate(ER(0:LX,-IZ:IZ), ET(0:LX,-IZ:IZ)
     .             ,EZ(0:LX,-IZ:IZ))
       allocate(PHI(0:LX,-IZ:IZ),RHOC(0:LX,-IZ:IZ))
       allocate(AT(-1:LX,-IZ:IZ),AZ(-1:LX,-IZ:IZ))
       allocate(ATOLD(-1:LX,-IZ:IZ),AZOLD(-1:LX,-IZ:IZ))
...
!#! Marco, initialize big arrays
       ...
       vt(:,:) = dcmplx(0.d0, 0.d0) 
       vt(:,:) = dcmplx(0.d0, 0.d0) 
       brn(:,:) = dcmplx(0.d0, 0.d0) 
       btn(:,:) = dcmplx(0.d0, 0.d0) 
       btn(:,:) = dcmplx(0.d0, 0.d0) 
       vrn(:,:) = dcmplx(0.d0, 0.d0) 
       vtn(:,:) = dcmplx(0.d0, 0.d0) 
       vtn(:,:) = dcmplx(0.d0, 0.d0) 
       er(:,:) = dcmplx(0.d0, 0.d0) 
       et(:,:) = dcmplx(0.d0, 0.d0) 
       et(:,:) = dcmplx(0.d0, 0.d0) 
...
!c se sim 2d inizializzato solo il modo m=m2d
       ...
       ! per perturbare solo alcuni modi, bisogna dargli lo JANZ!
         ...
         DO 113 N=IN0,IN1
          IF (I2D.EQ.1 .OR. I2D.EQ.2) THEN
       	   WRITE(6,*) '2D - perturbo il modo',M,n2d*m
          else
...
!       write(*,*) 'DEBUG-1',j_mp,j_mpold
       ...
       !irrotational MP of the selected helicity
        if (.not. IBAND) then
         if (mp_perc .ne. 0.d0) then
          ...
          call apply_irrot_bc(amp_a,m_mp,n_mp,ph_mp,bbr,bbt,bbz)
          do ix=0, lx 
           br(ix,j_mp) = bbr(ix)
     ...
     .                           +  BESSI(m_mpold-1, n_mpold/RR))
           ...
           if (amp_old .eq. 0.d0) then
            ...
            call apply_irrot_bc(amp_old,m_mpold,
...
907     CONTINUE                                                       
         DO 908 IX=0,LX                                                 
              ...
              BTN(IX,J) = UU(2,IX)                                      
908     CONTINUE                                                       
!        write(*,*),'pre-solenoidalita2 ,it= ',it
!     .             ,'btn=',btn(50,j0),' bt=',bt(50,j0)
...
!#! inizioparte2
            call apply_irrot_bc(amp_a,m_mp,n_mp,phmp,bbr,bbt,bbz)
...
1111         write(6,*) ' zeit= ',zeit,'  stato finale:'
       ...
       deallocate(br,bt,bz,vr,vt,vz)
       deallocate(brn,btn,bzn,vrn,vtn,vzn)
       deallocate(vrp,vtp,phi,rhoc)
       deallocate(jr,jt,jz,er,et,ez)
       deallocate(at,az,atold,azold)
