pro antifft_reconst_tok, path, str, d3, hel ,nth, nzz, itp0, itp1, deltaitp, mymin, mymax
...
;#; begin 3d part
  ...
  outfile = path+'dat/itp/'+strtrim(string(itp(q),format='(i0)'))+'/antifft_'+str+'_'+strtrim(string(nth,format='(i0)'))+'x'+strtrim(string(nzz,format='(i0)'))+'myminmax'+strtrim(string(mymin,format='(i0)'))+strtrim(string(mymax,format='(i0)'))+'.sav'
print,outfile
...
;            print,im,in,antifft_t[q,0:1,t,z], '   ', pdbt[itp[q],0:1,jmn]
  ...
  if (str eq 'b') then begin
   ...
   bt = reform(antifft_t)
   bz = reform(antifft_z)
   save,filename=outfile,br,bt,bz,iitp,pror1,pror2,vecth,vecz,ptime
;   save,filename=outfile,br,itp,pror1,pror2,vecth,vecz,ptime
  ...
  if (str eq 'v') then begin
   ...
   vt = reform(antifft_t)
   vz = reform(antifft_z)
   save,filename=outfile,vr,vt,vz,iitp,pror1,pror2,vecth,vecz,ptime
  endif
  if (str eq 'j') then begin
   jr = reform(antifft_r)
   jt = reform(antifft_t)
   jz = reform(antifft_z)
   save,filename=outfile,jr,jt,jz,iitp,pror1,pror2,vecth,vecz,ptime
