...
;#; Marco, computes the "radial displacement" associated to one MHD mode or to a set of MHD modes, based on the formula csi_r^(mn) = (r*br^(mn)/(i*Bt00*(m+nq)))
 path = '//ricercatori/ft/specyl/archive2/rfp/3d/theta16/parete_lontana/s1000p0100_parete_resistiva_tauw1d4_ridw1.2/'
...
; nzz =  128
 ...
 ;#; compute axisymmetric safety factor
  ...
  outfile = path+'dat/itp/'+strtrim(string(itp(q),format='(i0)'))+'/displacement_m1_moremodes.sav'
  print,outfile
