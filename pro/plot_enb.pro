pro plot_enb, path,d3,tmin,tmax
...
if (result eq 1) then begin
 ...
 if ((nzcon[0] - nz[0]) ge 2) then begin
  print,'spectrum.sav error in plot_enb.pro'
  stop
 endif
endelse
...
help,b_comp
yrange=[0.,1.2*max(b_comp[tmin:tmax,jin:jfin])]
print,'b_en yange',yrange
xrange=[min(tt),max(tt)]
xrange=[tt(tmin),tt(tmax)]
plot,tt[tmin:tmax],b_comp[tmin:tmax,jin+2],/nodata,ytitle='W!DM!N',yr=yrange,ystyle=1,xr=xrange,xstyle=1
if (d3 eq 1) then begin
for q=jin,jfin do begin
 oplot,tt[tmin:tmax],b_comp[tmin:tmax,q],col=colors(q-jin),thick=th
  print,'first',q,colors(q-jin)
endfor
...
xyouts,xbase,ybase,'(1,h)',col=colors(1-jin),/norm
xyouts,xbase,ybase-deltay,'(2,2h)',col=colors(2-jin),/norm
xyouts,xbase+deltax,ybase,'(3,3h)',col=colors(3-jin),/norm
...
xrange=[tt(tmin),tt(tmax)]
yrange=[1.e-8,1.e0]
yrange=[1.e-8,2.*max(b_comp[tmin:tmax,jin:jfin])]
;yrange=[1.e-7,1.e-1]
print,'b_en yange2',yrange
if (d3 eq 1) then begin
 plot,tt[tmin:tmax],b_comp[tmin:tmax,jin+2],/nodata,ytitle='W!DM!N',yr=yrange,ystyle=1,xr=xrange,xstyle=1,/ylog,xtitle='time ('+greektau+'!DA!N)',position=[0.15,0.11,0.94,0.94]
...
;#; 2d part
plot,[0.],/nodata,ytitle='W!DM!N',yr=yrange,ystyle=1,xr=xrange,xstyle=1,/ylog,xtitle='time ('+greektau+'!DA!N)',position=[0.15,0.11,0.94,0.94]
 oplot,tt[tmin:tmax],b_comp[tmin:tmax,0],col=0,thick=th
for q=jin,jfin do begin
 oplot,tt[tmin:tmax],b_comp[tmin:tmax,q],col=colors(q-jin),thick=th
endfor
...
filename=path+'dat/log_mag_en_itp.eps'
phd_graphopener,filename=filename,xsize=20,ysize=15
...
yrange=[1.e-6,1.e0]
yrange=[1.e-6,2.*max(b_comp[tmin:tmax,jin:jfin])]
xrange=[min(tt),max(tt)]
xrange=[tt(tmin),tt(tmax)]
tttt=indgen(fix(tmax)-fix(tmin))+fix(tmin)
;yrange=[1.e-7,1.e-1]
plot,tttt,b_comp[tmin:tmax,jin+2],/nodata,ytitle='W!DM!N',yr=yrange,ystyle=1,/ylog,xtitle='ITP',position=[0.15,0.11,0.94,0.94]
if (d3 eq 1) then begin
 oplot,tttt,b_comp[tmin:tmax,0],col=0,thick=th
for q=jin,jfin do begin
 oplot,tttt,b_comp[tmin:tmax,q],col=colors(q-jin),thick=th
endfor
for q=nmds-1l,0l,-1l do begin
 col=colors(q)
; print,'q',q,q/2,xbase+deltax*(q/2 + abs(q) mod 2),ybase-(q mod 2)*deltay
...
yrange=[0.,1.2*max(v_comp[tmin:tmax,jin:jfin])]
endif else begin
yrange=[0.,1.2*max(v_comp[10:tmax,jin:jfin])]
endelse
print,yrange
xrange=[min(tt),max(tt)]
xrange=[tt(tmin),tt(tmax)]
plot,tt[tmin:tmax],v_comp[tmin:tmax,jin+2],/nodata,ytitle='W!DK!N',yr=yrange,ystyle=1,xr=xrange,xstyle=1
if (d3 eq 1) then begin
for q=jin,jfin do begin
 oplot,tt[tmin:tmax],v_comp[tmin:tmax,q],col=colors(q-jin),thick=th
endfor
endif else begin
...
xrange=[tt(tmin),tt(tmax)]
yrange=[1.e-7,1.e-1]
plot,tt[tmin:tmax],v_comp[tmin:tmax,jin+2],/nodata,ytitle='W!DK!N',yr=yrange,ystyle=1,xr=xrange,xstyle=1,/ylog,position=[0.15,0.11,0.94,0.94]
if (d3 eq 1) then begin
...
xrange=[-30,10]
if (d3 ne 0) then begin
plot,modes0,b_comp[-1,0:nz(0)-1],/nodata,ytitle='W!DM!N',yr=yrange,ystyle=1,xr=xrange,xstyle=1,/ylog,xtitle='mode number',title='spectrum at final time'

oplot,modes0,b_comp[-1,1:nz(0)],psym=-symcat(16),syms=2.,linestyle=2,col=50
oplot,modes1,b_comp[-1,nz(0)+1:nz(0)+nz(1)-1],psym=-symcat(16),syms=2.,linestyle=0,col=190
...
;#; create *pdf output
filename=path+'dat/mag_en.eps'
infiles = [path+'dat/log_mag_en.eps', path+'dat/mag_en.eps', path+'dat/log_kin_en.eps', path+'dat/kin_en.eps', path+'dat/log_mag_en_itp.eps', path+'dat/tot_en.eps', path+'dat/enb_spect.eps']
